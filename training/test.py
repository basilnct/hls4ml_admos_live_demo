    ...
    for idx, target_dir in enumerate(dirs):
        ...
        for id_str in machine_id_list:
            ...
            for file_idx, file_path in tqdm(enumerate(test_files), total=len(test_files)):
                try:
                    data = com.file_to_vector_array(file_path=file_path,
            ...
            k_fpr, k_tpr, k_threshold = metrics.roc_curve(y_true, y_pred)
            # finding the best threshold to use for abnormaly detection using Gmean
            gmeans = numpy.sqrt(k_tpr * (1 - k_fpr))
            ix = numpy.argmax(gmeans)
            print('Best Threshold=%f, G-Mean=%.3f' % (k_threshold[ix], gmeans[ix]))
